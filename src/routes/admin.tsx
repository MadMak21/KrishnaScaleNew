...
function fileToBase64(file: File): Promise<string> {
  return new Promise((resolve, reject) => {
    // If the file is not an image (safety check), read as-is
...
function AdminPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (username.trim() === "madhav" && password.trim() === "MADmak@21") {
      login();
      setError("");
    } else {
        ...
        <form onSubmit={handleLogin} className="bg-black/50 p-8 rounded-2xl border border-white/10 w-full max-w-md space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="relative">
              <input type={showPassword ? "text" : "password"} placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white pr-12" />
